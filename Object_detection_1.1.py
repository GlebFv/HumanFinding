...
time_a = 0
time_t = 0
ftp = FTP('195.69.187.77', user='taxiuser', passwd='XJnLe7XnYZWt3Eh')
...
while True:
    ...
    if ''.join(str(datetime.datetime.now().time()).split(':')[:2]) == '0800':
        day_cat = int(ftp.nlst()[-2])
        break

while True:
    lst = ftp.nlst(str(day_cat))
    print(lst)
    image_analis()
    b_time_sender = []
    print('llll', len(lst))
    for i in range(len(lst)):
        print(i)
        print('worked_time', worked_time_v2)
        try:
            out = 'office.jpg'
            print("получил изображение")
            print(lst[i], day_cat)
            with open(out, 'wb') as f:
                ftp.retrbinary('RETR ' + f'{lst[i]}', f.write)
        except:
            pass
        result = image_analis()
        if result != 0:
